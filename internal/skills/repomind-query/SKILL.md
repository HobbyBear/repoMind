...
### 4c：必要时使用 /graphify 深入分析

如果需求涉及跨模块调用链、数据流、依赖关系等需要深度分析的场景，使用 `/graphify query "<问题>"` 查询代码图谱。

`/graphify` 会从 `graphify-out/graph.json` 读取图谱数据（该文件由 graphify skill 生成和管理），提供代码间的调用关系、社区结构等信息。
